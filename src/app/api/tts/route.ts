...
export async function POST(request: NextRequest) {
  try {
    const { questionId, teacherExplanation, showOfficialExplanation, showTip, voiceStyle, ttsVoice, force } = await request.json();
    if (!questionId) {
    ...
    const [result, bridgeDurations] = await Promise.all([
      generateTTSForQuestion(questionId, {
        teacherExplanation, showOfficialExplanation, showTip, voiceStyle, ttsVoice, force,
      }),
